...
impl LuaUserData for Drive {
    fn add_methods<M: LuaUserDataMethods<Self>>(methods: &mut M) {
        ...
        methods.add_async_method(
            "upload_file",
            |_, this, (folder_id_or_path, file_val): (String, LuaAnyUserData)| async move {
...
async fn resolve_path(state: Arc<GmailState>, email: &str, path: &str) -> LuaResult<String> {
    ...
    for part in parts {
        ...
        let files = json
            .get("files")
            .and_then(|f| f.as_array())
            .ok_or_else(|| LuaError::RuntimeError("Invalid response from Drive API".into()))?;
        ...
        current_id = files[0]
            .get("id")
            .and_then(|v| v.as_str())
            .ok_or_else(|| LuaError::RuntimeError("Missing ID in response".into()))?
            .to_string();
    }
    Ok(current_id)
}
